...
void GPIO_PullUp(uint32_t GPIO_){
	...
	if(rtc_pins[GPIO_]!=0){
		HWREG32(dir_rtcio_touch_pad[rtc_pins[GPIO_]]) |=(1<<27);
		HWREG32(dir_rtcio_touch_pad[rtc_pins[GPIO_]]) &= ~(1<<28);
	}
	else {
		HWREG32(GPIO_PINX_MUX_REG[GPIO_]) |=(1<<PULL_UP);
		HWREG32(GPIO_PINX_MUX_REG[GPIO_]) &= ~(1<<PULL_DOWN);
	}
}
...
int GPIO_Leer(uint32_t GPIO) {
    ...
    if (GPIO >= GPIO_32 && GPIO <= GPIO_39) {
        ...
        valor_leido = (GPIO_IN1_REG & (1 << (GPIO - 32))) ? 1 : 0;
    }
    // Si el pin no está en el rango de GPIO_32 a GPIO_39
        ...
        valor_leido = (GPIO_IN_REG & (1 << GPIO)) ? 1 : 0;
    }
